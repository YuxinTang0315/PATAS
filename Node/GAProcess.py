...
class GA():
	...
	def run(self, gen=50):
		...
		for n in range(1, gen+1):
			...
			for i in range(mutation_times):
				...
				for I in child:
					if I.evaluation is None:
						I.evaluation = MODEL(I.solution.reshape(1, -1)).item()
						I.fitness = I.evaluation
					children.append(I)

			# select representative, temporally ignore
			slt_children = sorted(children, key=lambda x: x.fitness, reverse=True) # Light copy
			...
			print("running_time:", n, "best_evaluation:", self.population.best.evaluation)
