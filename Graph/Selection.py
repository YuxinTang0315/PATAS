...
class RouletteWheelSelection(Selection):
	...
	def select(self, population):
		fitness = np.exp(np.array([I.fitness/10 for I in population.individuals]).astype(float))
		fitness2 = fitness / fitness.sum()
   
		selected_individuals = np.random.choice(population.individuals, population.size, p=fitness2.astype(np.float_))
