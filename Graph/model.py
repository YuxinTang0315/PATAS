...
from graphpreprocessing import GRAPHPREPROCESS
...
class NetworkCORA(nn.Module):
	def __init__(self, mat, args, input_dim, genotype, num_class, in_degree):
		...
		self.g_process = GRAPHPREPROCESS(hidden_size=hidden_dim_prev)
		self.g_process_norm = nn.BatchNorm1d(hidden_dim_prev)
		# self.g_process= nn.Sequential(
			# GRAPHPREPROCESS(hidden_dim_prev),
			# nn.BatchNorm1d(hidden_dim_prev)
		# )
	...
	def forward(self, batch, max_node):
		# s0 = s1 = self.stem(batch.x)
		# print(s1.size())
		batch.x, batch.batch, _ = self.g_process(batch.x, batch.batch)
		# batch.x, _, _ = self.g_process(batch.x, batch.batch)
		s0 = s1 = self.g_process_norm(batch.x)
		for i in range(max(batch.batch)):
				a = torch.nonzero(batch.batch==i)[-1].item()
				b = torch.nonzero(batch.batch==i)[-2].item()
				if i==0:
						for j in range(0, b):
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([b, j]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([j, b]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([a, j]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([j, a]).cuda().unsqueeze(1)), dim=1)
								for k in range(4):
										batch.edge_attr = torch.cat((batch.edge_attr, torch.tensor([0, 0, 0]).reshape(1, 3).cuda()), dim=0)
				else:
						for j in range(torch.nonzero(batch.batch==i-1)[-1].item()+1, b):
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([b, j]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([j, b]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([a, j]).cuda().unsqueeze(1)), dim=1)
								batch.edge_index = torch.cat((batch.edge_index, torch.tensor([j, a]).cuda().unsqueeze(1)), dim=1)
								for k in range(4):
										batch.edge_attr = torch.cat((batch.edge_attr, torch.tensor([0, 0, 0]).reshape(1, 3).cuda()), dim=0)
		in_degree = degree(batch.edge_index[0].long(), batch.x.shape[0]).int().cuda()
		out_degree = in_degree
   
		# s0 = s1 = batch.x
		edge_attr = self.bond_encoder(batch.edge_attr.long())
		for i, cell in enumerate(self.cells):
			s0, s1 = s1, cell(s0, s1, batch.edge_index.long(), edge_attr, in_degree, out_degree, self.mat, batch.batch, max_node)
		h_graph = self.pool(s1, batch.batch)
