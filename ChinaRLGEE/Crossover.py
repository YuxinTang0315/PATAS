...
class DecimalCrossover(Crossover):
	...
	def cross_individuals(individual_a, individual_b, pos, alpha):
		...
		# cross value
		temp = individual_a.solution * pos
		temp2 = individual_b.solution * pos
		row, col = np.nonzero(pos)

		new_value_a = np.copy(individual_a.solution)
		new_value_b = np.copy(individual_b.solution)
		for r, c in zip(row, col):
			new_value_a[r, c] = temp2[r, c]
			new_value_b[r, c] = temp[r, c]
